...
@add_start_docstrings("""DistilBert Model with a span classification head on top for extractive question-answering tasks like SQuAD (a linear layers on top of
                         the hidden-states output to compute `span start logits` and `span end logits`). """,
                      DISTILBERT_START_DOCSTRING, DISTILBERT_INPUTS_DOCSTRING)
class DistilBertForQuestionAnsweringGeneralized(DistilBertPreTrainedModel):
    r"""
        **start_positions**: (`optional`) ``torch.LongTensor`` of shape ``(batch_size,)``:
            Labels for position (index) of the start of the labelled span for computing the token classification loss.
            Positions are clamped to the length of the sequence (`sequence_length`).
    ...
    def __init__(self, config):
        super(DistilBertForQuestionAnsweringGeneralized, self).__init__(config)

        self.distilbert = DistilBertModel(config)
        self.qa_outputs = nn.Linear(config.dim, config.num_labels)
        self.att_outputs = nn.Linear(config.dim, 1)
        assert config.num_labels == 2
        self.dropout = nn.Dropout(config.qa_dropout)

        # Todo: change predictions
        self.cls_head_sizes = config.cls_head_sizes # add this to config before calling
        self.answer_classes = torch.nn.ModuleList(
            [nn.Linear(config.hidden_size, k, bias=False) for k in self.cls_head_sizes]
        )
    ...
    def forward(self, input_ids, attention_mask=None, head_mask=None, start_positions=None, end_positions=None,
                head_idx=None, span_loss=None, is_impossible=None):
        ...
        logits = self.qa_outputs(hidden_states)  # (bs, max_query_len, 2)
        start_logits, end_logits = logits.split(1, dim=-1)
        att_logits = self.att_outputs(hidden_states) # Todo: added attention

        att_logits_sigmoid = torch.sigmoid(att_logits).squeeze(-1)
        att_states = torch.einsum("blh,bl->bh", hidden_states, att_logits_sigmoid)
        ...
        if start_positions is not None and end_positions is not None:
            ...
            bch_size = start_logits.shape[0]
            true_range = torch.cat(
                [torch.cat((torch.zeros(start_positions[i]), torch.ones(1 + end_positions[i] - start_positions[i]),
                            torch.zeros(ignored_index - 1 - end_positions[i]))).reshape(1, -1) for i in range(bch_size)]
            )
            att_loss = att_loss_fct(att_logits_sigmoid, true_range)
